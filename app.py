...
enginesize = st.number_input("Ukuran Mesin/Engine Size(in³)", float(X['enginesize'].min()), float(X['enginesize'].max()), float(X['enginesize'].mean()))
horsepower = st.number_input("Tenaga Mesin/Horsepower(HP)", float(X['horsepower'].min()), float(X['horsepower'].max()), float(X['horsepower'].mean()))
curbweight = st.number_input("Berat Mobil Kosong/Curb Weight(KG)", float(X['curbweight'].min()), float(X['curbweight'].max()), float(X['curbweight'].mean()))
carwidth = st.number_input("Lebar Mobil/Car Width(INCH)", float(X['carwidth'].min()), float(X['carwidth'].max()), float(X['carwidth'].mean()))
highwaympg = st.number_input("Efisiensi Jalan Tol/Highway MPG(MPG)", float(X['highwaympg'].min()), float(X['highwaympg'].max()), float(X['highwaympg'].mean()))
citympg = st.number_input("Efisiensi Dalam Kota/City MPG(MPG)", float(X['citympg'].min()), float(X['citympg'].max()), float(X['citympg'].mean()))
